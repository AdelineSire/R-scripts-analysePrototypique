...
dico <- read.csv2(file = "dico_induits.csv", header = T, sep = ";")

# Crée un vecteur pour les formes et les induits
formes <- as.character(dico$Induits)
induits <- as.character(dico$Familles)
...
write.table(synonymes, file = "familles.csv", quote = F, col.names = F, row.name = F, sep=";")

# Crée un dossier Data_c dans le dossier Data
dir.create("Data_c")

# Sort un fichier evocs_induits
write.table(evocs, file = "Data_c/evocs_familles.csv", quote = F, row.names = F, col.names = T, sep=";")